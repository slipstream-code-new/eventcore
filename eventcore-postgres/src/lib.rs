        ...
        + 'static,
{
    ...
    pub async fn initialize(&self) -> Result<(), PostgresError> {
        ...
        let result = async {
            ...
            // Create simplified row-level trigger for basic validation
            sqlx::query(
                r"
                CREATE OR REPLACE FUNCTION check_basic_event_constraints() RETURNS TRIGGER AS $$
                DECLARE
                ...
                BEGIN
                    ...
                    -- Note: event_id is automatically generated by column default
                    -- Comprehensive gap detection is handled by statement-level trigger
            ...
            // Create function for comprehensive batch gap detection
            sqlx::query(
                r"
                CREATE OR REPLACE FUNCTION validate_batch_versions() RETURNS TRIGGER AS $$
                DECLARE
                    stream_record RECORD;
                    previous_max_version BIGINT;
                    min_new_version BIGINT;
                    max_new_version BIGINT;
                ...
                BEGIN
                    ...
                    -- Get all streams that had events inserted in this statement
                    -- We'll identify them by events whose xmin matches current transaction
                    FOR stream_record IN
                        SELECT DISTINCT stream_id
                        FROM events
                        WHERE xmin::text::bigint = current_txid
                    LOOP
                        -- Get the previous max version (before this transaction)
                        SELECT COALESCE(MAX(event_version), 0) INTO previous_max_version
                        FROM events 
                        WHERE stream_id = stream_record.stream_id
                        AND xmin::text::bigint != current_txid;
                        
                        -- Get the range of new versions for this stream in this transaction
                        SELECT MIN(event_version), MAX(event_version), COUNT(*) 
                        INTO min_new_version, max_new_version, actual_count
                        FROM events 
                        WHERE stream_id = stream_record.stream_id
                        AND xmin::text::bigint = current_txid;
                        
                        -- Validate that new versions start immediately after previous max
                        IF min_new_version != previous_max_version + 1 THEN
                            RAISE EXCEPTION 'Version gap detected for stream %: expected first new version %, got %',
                                stream_record.stream_id, previous_max_version + 1, min_new_version
                                USING ERRCODE = '40001';
                        END IF;
                        
                        -- Validate no gaps within the batch
                        expected_count := max_new_version - min_new_version + 1;
                        IF actual_count != expected_count THEN
                            RAISE EXCEPTION 'Version gaps detected within batch for stream %: expected % contiguous versions, got %',
                                stream_record.stream_id, expected_count, actual_count
                                USING ERRCODE = '40001';
                        END IF;
                    END LOOP;
            ...
            sqlx::query("DROP TRIGGER IF EXISTS check_version_gaps_after_insert ON events")
                .execute(self.pool.as_ref())
                .await
                .map_err(PostgresError::Connection)?;

            sqlx::query("DROP TRIGGER IF EXISTS enforce_basic_event_constraints ON events")
                .execute(self.pool.as_ref())
                .await
                .map_err(PostgresError::Connection)?;

            sqlx::query("DROP TRIGGER IF EXISTS validate_batch_versions_after_insert ON events")
                .execute(self.pool.as_ref())
                .await
                .map_err(PostgresError::Connection)?;

            // Create row-level trigger for basic validation (ExpectedVersion::New)
            sqlx::query(
                r"
                CREATE TRIGGER enforce_basic_event_constraints
                    BEFORE INSERT ON events
                    FOR EACH ROW
                    EXECUTE FUNCTION check_basic_event_constraints()
                ",
            )
            .execute(self.pool.as_ref())
            .await
            ...
            // Create statement-level trigger for comprehensive gap detection
            sqlx::query(
                r"
                CREATE TRIGGER validate_batch_versions_after_insert
                    AFTER INSERT ON events
                    FOR EACH STATEMENT
                    EXECUTE FUNCTION validate_batch_versions()
                ",
            )
            .execute(self.pool.as_ref())
            .await
