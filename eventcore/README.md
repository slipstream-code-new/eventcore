...
impl Command for TransferMoney {
    ...
    async fn handle(
        ...
        // Clean event generation with emit! macro
        let mut events = vec![];
...
async fn handle(
    ...
    // Validate with require! macro
    require!(state.is_valid(), "Invalid state");
    
    // Discover we need approval for large amounts
    if input.amount > Money::from_cents(1000000)? {  // $10,000
...
// Create command with derive macro
#[derive(Command)]
struct OpenAccount {
    #[stream]
    account_id: StreamId,
...
// Execute with minimal boilerplate
