...
impl OpenAccount {
    /// Creates a new `OpenAccount` command with validation
    ///
...
impl CommandLogic for OpenAccount {
    ...
    async fn handle(
        ...
        if state.existing_accounts.contains_key(&self.account_id) {
            return Err(CommandError::BusinessRuleViolation(
                BankingError::AccountAlreadyExists(self.account_id.clone()).to_string(),
            ));
        }
        ...
        let event = AccountOpened {
            ...
            initial_balance: self.initial_deposit,
        };
...
impl TransferMoneyCommand {
    /// Creates a new transfer money command with validation
    ///
    /// # Errors
    ///
    /// Returns an error if from and to accounts are the same
    ...
    async fn handle(
        ...
        let from_balance = state.balances.get(&self.from_account).ok_or_else(|| {
            CommandError::BusinessRuleViolation(
                BankingError::AccountNotFound(self.from_account.clone()).to_string(),
            )
        })?;

        if !state.balances.contains_key(&self.to_account) {
            return Err(CommandError::BusinessRuleViolation(
                BankingError::AccountNotFound(self.to_account.clone()).to_string(),
            ));
        }

        // Check sufficient funds
        if from_balance.subtract(&self.amount).is_err() {
            return Err(CommandError::BusinessRuleViolation(
...
impl SafeTransferMoneyCommand {
    /// Creates a new SafeTransferMoneyCommand with compile-time type safety
    pub fn new(
        transfer_id: TransferId,
        source: SourceAccount,
        target: TargetAccount,
    ...
    /// Creates from separate account IDs with validation
    pub fn from_account_ids(
        transfer_id: TransferId,
        from_account: AccountId,
        to_account: AccountId,
...
impl CommandStreams for SafeTransferMoneyCommand {
    type StreamSet = ();

    fn read_streams(&self) -> Vec<StreamId> {
        // Read from both account streams and the transfer log
        vec![
            StreamId::try_new(format!("account-{}", self.source.account_id())).unwrap(),
            StreamId::try_new(format!("account-{}", self.target.account_id())).unwrap(),
            StreamId::try_new("transfers".to_string()).unwrap(),
        ]
    }
}
...
#[async_trait]
impl CommandLogic for SafeTransferMoneyCommand {
    type State = TransferMoneyState;
    type Event = BankingEvent;
    ...
    async fn handle(
        ...
        let from_balance = state
            .balances
            .get(self.source.account_id())
            .ok_or_else(|| {
                CommandError::BusinessRuleViolation(
                    BankingError::AccountNotFound(self.source.account_id().clone()).to_string(),
                )
            })?;

        if !state.balances.contains_key(self.target.account_id()) {
            return Err(CommandError::BusinessRuleViolation(
                BankingError::AccountNotFound(self.target.account_id().clone()).to_string(),
            ));
        }
        ...
        if from_balance.subtract(&amount).is_err() {
            return Err(CommandError::BusinessRuleViolation(
                BankingError::InsufficientFunds {
                    account: self.source.account_id().clone(),
                    balance: *from_balance,
                    requested: amount,
                }
                .to_string(),
...
impl CommandStreams for TypeSafeTransferCommand {
    type StreamSet = ();

    fn read_streams(&self) -> Vec<StreamId> {
        // Read from both account streams and the transfer log
        vec![
            StreamId::try_new(format!("account-{}", self.accounts.source().account_id())).unwrap(),
            StreamId::try_new(format!("account-{}", self.accounts.target().account_id())).unwrap(),
            StreamId::try_new("transfers".to_string()).unwrap(),
        ]
    }
}
...
#[async_trait]
impl CommandLogic for TypeSafeTransferCommand {
    type State = TransferMoneyState;
    type Event = BankingEvent;
    ...
    async fn handle(
        ...
        let from_balance = state
            .balances
            .get(self.accounts.source().account_id())
            .ok_or_else(|| {
                CommandError::BusinessRuleViolation(
                    BankingError::AccountNotFound(self.accounts.source().account_id().clone())
                        .to_string(),
                )
            })?;
        ...
        if !state
            .balances
            .contains_key(self.accounts.target().account_id())
        {
            return Err(CommandError::BusinessRuleViolation(
                BankingError::AccountNotFound(self.accounts.target().account_id().clone())
                    .to_string(),
            ));
        }

        // Check sufficient funds (the only runtime validation needed!)
        let amount = self.amount.money();
        if from_balance.subtract(&amount).is_err() {
            return Err(CommandError::BusinessRuleViolation(
                BankingError::InsufficientFunds {
                    account: self.accounts.source().account_id().clone(),
                    balance: *from_balance,
                    requested: amount,
                }
                .to_string(),
        ...
        let event = MoneyTransferred {
            transfer_id: self.transfer_id.clone(),
            from_account: self.accounts.source().account_id().clone(),
            to_account: self.accounts.target().account_id().clone(),
            amount,
            description: self.description.clone(),
        };
        ...
        let events = vec![
            StreamWrite::new(
                &read_streams,
                StreamId::try_new(format!("account-{}", self.accounts.source().account_id()))
                    .unwrap(),
                BankingEvent::from(event.clone()),
            )?,
            StreamWrite::new(
                &read_streams,
                StreamId::try_new(format!("account-{}", self.accounts.target().account_id()))
                    .unwrap(),
                BankingEvent::from(event.clone()),
            )?,
            StreamWrite::new(
...
mod tests {
    ...
    fn open_account_input_valid() {
        let _command = OpenAccount::new(AccountId::generate(), create_test_holder(), Money::zero());
